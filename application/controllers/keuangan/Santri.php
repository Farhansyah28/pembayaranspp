...
class Santri extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->db->select('santri.*, angkatan.nama as angkatan_nama, wali_user.username as wali_username');
        $this->db->from('santri');
    ...
    public function edit($id)
    {
        ...
        $data['s'] = $this->db->where('id', $id)->get('santri')->row();
        ...
        $this->render('keuangan/santri/edit', $data);
    }

    public function update($id)
    {
        $santri = $this->db->where('id', $id)->get('santri')->row();
        if (!$santri)
            show_404();
        ...
        if ($this->input->post('nis') != $santri->nis) {
            $this->form_validation->set_rules('nis', 'NIS', 'required|is_unique[santri.nis]');
        }
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            $this->db->where('id', $id)->update('santri', $data);
            $this->log_activity('Memperbarui santri: ' . $data['nama'], 'Santri');
    ...
    public function delete($id)
    {
        ...
        // Delete santri logic
        // Note: Cascading usually handled by DB, but we log it
        $this->db->where('id', $id)->delete('santri');
        $this->log_activity('Menghapus santri: ' . $santri->nama, 'Santri');
